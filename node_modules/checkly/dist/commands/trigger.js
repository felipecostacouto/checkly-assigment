...
class Trigger extends authCommand_1.AuthCommand {
    async run() {
        ...
        const { location: runLocation, 'private-location': privateRunLocation, config: configFilename, tags: targetTags, timeout, verbose: verboseFlag, record: shouldRecord, reporter: reporterFlag, env, 'env-file': envFile, 'test-session-name': testSessionName, retries, } = flags;
        ...
        const testRetryStrategy = this.prepareTestRetryStrategy(retries, (_c = checklyConfig === null || checklyConfig === void 0 ? void 0 : checklyConfig.cli) === null || _c === void 0 ? void 0 : _c.retries);
        const repoInfo = (0, util_1.getGitInformation)();
        const ciInfo = (0, util_1.getCiInformation)();
        const runner = new trigger_runner_1.default(config_1.default.getAccountId(), timeout, verbose, shouldRecord, location, (_d = targetTags === null || targetTags === void 0 ? void 0 : targetTags.map((tags) => tags.split(','))) !== null && _d !== void 0 ? _d : [], Object.entries(envVars).map(([key, value]) => ({ key, value })), repoInfo, ciInfo.environment, testSessionName, testRetryStrategy);
